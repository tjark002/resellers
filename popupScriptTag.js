...
ready(function(){
    ...
    formPopup.innerHTML = "<form action='' class='formContainer'><h2>Please Log in</h2><label for='email'>  <strong>Reseller-ID</strong></label><input type='text' id='resellerid' placeholder='Die ID Deines Resellers' name='resellerid' required><button onclick='sendResellerIdToBackend()' class='btn'>Bestätigen</button><!--<button type='button' class='btn cancel' onclick='popDown()'>Close</button>--></form>";
